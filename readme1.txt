...
・デフォルトでレス不可、画像返信不可にしてあります。
　１ページ１０投稿表示でログ行数は１０１０行、最大１０１ページにしてありますが
　ここらへんはhokan.php適宜弄ってくださいね

define(PAGE_DEF, '10');             // 一ページに表示する記事
define(FOLL_ADD, '1010');           // 以下省略（一ページに表示する記事×指定頁数＝設定数
define(LOG_MAX,  '1010');           // ログ最大行数

　レス可能にするには↓331行目行頭の//を削除、画像返信許可設定は上部項目で許可してください。

//    if(!$resno){ $dat.="[<a href=".PHP_SELF."?res=$no>返信</a>]<br>"; }


・viewa.phpとhokan.phpのログファイル名(img.log/tree.log)は
　必ず同じ名称に合わせてください。
...
　「・保存内容に該当しない画像・ふたばネタ以外の画像は貼らないようお願いします。」;
　　↓
 hokan.phpの138行目
$addinfo='<br><li>保存内容に該当しない画像・ふたばネタ以外の画像は貼らないようお願いします。';
